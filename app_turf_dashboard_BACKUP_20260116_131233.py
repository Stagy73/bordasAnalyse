...
def main():
        ...
        with st.sidebar:
            with st.spinner("Import en cours..."):
                ...
                if stats and not stats.get('errors'):
                    ...
                    st.info(f"📝 {stats['courses']} courses importées")
                    st.info(f"🐴 {stats['partants']} partants ajoutés")
                else:
    ...
    if df.empty:
        st.warning(f"⚠️ Aucune donnée pour la période {date_debut} → {date_fin}")
