...
class TurfDatabase:
    ...
    def create_course(self, course_code: str, reunion_id: int, numero_course: int,
        ...
        # CRITIQUE : Chercher avec course_code ET reunion_id
        self.cursor.execute(
            "SELECT id FROM courses WHERE course_code = ? AND reunion_id = ?", 
            (course_code, reunion_id)
        )
