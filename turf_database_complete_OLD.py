...
class TurfDatabase:
    ...
    def _create_all_tables(self):
        ...
        self.cursor.execute("""
            CREATE TABLE IF NOT EXISTS reunions (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                reunion_code TEXT NOT NULL UNIQUE,
                date DATE NOT NULL,
        ...
        try:
            ...
            for course_code in df['Course'].unique():
                ...
                reunion_id = self.get_or_create_reunion(
                    reunion_code, date_reunion, hippodrome_id
                )
                ...
                course_id = self.create_course(
                    ...
                    distance=int(course_df['distance'].iloc[0]) if 'distance' in course_df.columns else None,
                    allocation=float(course_df['allocation'].iloc[0]) if 'allocation' in course_df.columns else None,
                ...
                for _, row in course_df.iterrows():
                    ...
                    self.create_partant(
                        ...
                        cote_pmu=float(row['Cote']) if 'Cote' in row and pd.notna(row['Cote']) else None,
                        cote_bzh=float(row['Cote BZH']) if 'Cote BZH' in row and pd.notna(row['Cote BZH']) else None,
                        musique=row.get('Musique'),
                        ia_data={
                            'ia_gagnant': row.get('IA_Gagnant'),
                            'ia_couple': row.get('IA_Couple'),
                            'ia_trio': row.get('IA_Trio'),
                            'note_ia': row.get('Note_IA_Decimale')
                        },
                        performance_data={
                            'turf_points': row.get('Turf Points'),
                            'tpch_90': row.get('TPch 90'),
                            'tpj_365': row.get('TPJ 365')
                        }
                    )
                    stats['partants'] += 1
    ...
    def get_or_create_reunion(self, reunion_code: str, date: date, hippodrome_id: int) -> int:
        ...
        self.cursor.execute(
            "SELECT id FROM reunions WHERE reunion_code = ? AND date = ?",
            (reunion_code, date)
        )
        row = self.cursor.fetchone()
